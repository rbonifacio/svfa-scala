...
//    println(body)
  ...
  private def invokeRule(callStmt: Statement, exp: InvokeExpr, caller: SootMethod, defs: SimpleLocalDefs): Unit = {
    ...
    if(analyze(callStmt.base) == SinkNode) {
      defsToCallOfSinkMethod(callStmt, exp, caller, defs) // update EDGE(S) between "declaration stmt(s) for args" and "call-site stmt" (current stmt)
    ...
    body.getUnits.forEach(s => {
      if(isThisInitStmt(exp, s)) {
        ...
        defsToCallSite(caller, callee, calleeDefs, callStmt.base, s) //create an "edge" FROM the stmt where the return variable was defined and "call site stmt"
      }
      else if(isReturnStringStmt(callStmt.base, s)) { // return "<string>"
        stringToCallSite(caller, callee, callStmt.base, s) //create EDGE between "return string stmt " and "call site stmt"
      }
    })
...
//          updateGraph(sourceNode, targetNode) // add comment
  ...
  private def defsToCallSite(caller: SootMethod, callee: SootMethod, calleeDefs: SimpleLocalDefs, callStmt: soot.Unit, retStmt: soot.Unit) = {
    ...
    calleeDefs.getDefsOfAt(local, retStmt).forEach(sourceStmt => {
      ...
      svg.addEdge(source, target, csCloseLabel) //create EDGE between "definition stmt from return variable " and "call site stmt"
  ...
  private def stringToCallSite(caller: SootMethod, callee: SootMethod, callStmt: soot.Unit, retStmt: soot.Unit): Unit = {
    ...
    svg.addEdge(source, target) // create EDGE between "return stmt " and "call site stmt"
  }
    ...
    if(invokeExpr != null) {
      if(invokeExpr.getBase.isInstanceOf[Local]) {
        ...
        calleeDefs.getDefsOfAt(base, callStatement.base).forEach(sourceStmt => {
          ...
          svg.addEdge(source, target, csOpenLabel) // add comment
        })
      }
    }
  }
   ...
   * An "open" CS is added
   *  - Caller method
   *  - stmt where the callee method is called (stmt is in caller method)
   *  - Callee method
   */
  ...
  private def defsToCallOfSinkMethod(stmt: Statement, exp: InvokeExpr, caller: SootMethod, defs: SimpleLocalDefs) = {
    ...
    exp.getArgs.stream().filter(a => a.isInstanceOf[Local]).forEach(a => {
      ...
      defs.getDefsOfAt(local, targetStmt).forEach(sourceStmt => {
        ...
        updateGraph(source, target) // update EDGE(S) between "declaration stmt(s) for args" and "call-site stmt" (current stmt)
      })
