...
abstract class JSVFA
  ...
  def traverse(
    ...
    (left, right) match {
      ...
      case (p: InstanceFieldRef, _: Local) =>
        storeRule(assignStmt.stmt, p, method, defs)
  ...
  def storeArrayRule(assignStmt: AssignStmt) {
    val l = assignStmt.stmt.getLeftOp
      .asInstanceOf[JArrayRef]
      .getBase
      .asInstanceOf[Local]
      ...
      if (invokeExpr.getBase.isInstanceOf[Local]) {
        ...
        calleeDefs
          ...
          .forEach(sourceStmt => {
            ...
            val csOpenLabel =
              createCSOpenLabel(caller, callStatement.base, callee)
  ...
  def createCSOpenLabel(
    ...
    CallSiteLabel(
      ContextSensitiveRegion(statement, callee.toString),
      CallSiteOpenLabel
    )
  }
        ...
        if (assignment.getLeftOp.isInstanceOf[InstanceFieldRef]) {
          ...
          if (
            pointsToAnalysis
              ...
              .hasNonEmptyIntersection(pointsToAnalysis.reachingObjects(local))
          ) {
            if (
              field.equals(
